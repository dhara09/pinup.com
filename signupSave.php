...
//header("location:signup.php?$encode");
//header("location:signup.php?$query");
//exit;
//$encode=rawurlencode($query);
//header("location:signup.php?$encode"); exit;
$errmsg='';
$check=1;
...
{ 
    ...
    if($check == 0){  
        ...
        header("Location:http://local.pinup.com/signup.php?error=$errmsg&na=$name&ln=$lname&em=$email&ad=$address&con=$contact&pas=$pass&cp=$cpass");
        exit;
    }
}
//header("location:signup.php?$query");
...
$encode=rawurlencode($query);
header("Location:sign.php?$encode"); */
